#!/bin/bash
# QMA installer for compiled release
...
ln -sf "$INSTALL_DIR/app.bin" "$BIN_PATH"

# Create desktop entry
echo "Creating desktop entry..."
cat > "$DESKTOP_FILE" <<EOL
[Desktop Entry]
Name=QMA - Quran Memorization Assistant
Comment=Quran Memorization Assistant
Exec=$BIN_PATH
Icon=$ICON_PATH
Terminal=false
Type=Application
Categories=Education;Audio;
Keywords=audio;quran;memorization;assistant;
StartupWMClass=app
EOL

# Fix icon path in the desktop file if needed
sed -i "s|/opt/QMA/assets/icon.png|$ICON_PATH|g" "$DESKTOP_FILE"
...
echo "Find in your application menu: QMA"
echo "=============================================="